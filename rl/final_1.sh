...
for robot in "${robots[@]}"; do
    for env in "${envs[@]}"; do
        for gin_file in "${gin_files[@]}"; do
            ...
            # 일단 테스트용!
            python toddlerbot/locomotion/train_mtjx.py --robot "$robot" --env "$env" --config-override "PPOConfig.num_timesteps=100,PPOConfig.num_evals=0" --gpu "0" --gin-files "$gin_file"
